...
$envFiles = @(
  (Join-Path $backendDir "config/.env"),
...
function Get-BackendPort {
  if (Is-PortListening 5000) {
    ...
    if ($port5000Cmd -match '-m app\.server') {
      return 5000
    }
...
# 2) Celery worker
if ($redisReady) {
  if (-not (Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -match "celery" -and $_.CommandLine -match "app\.server:celery_client" })) {
    $celeryCmd = "Set-Location '$backendDir'; `$env:CELERY_BROKER_URL='redis://127.0.0.1:6379/0'; `$env:CELERY_RESULT_BACKEND='redis://127.0.0.1:6379/1'; & '$pythonExe' -m celery -A app.server:celery_client worker -l info -P solo"
...
if (-not (Is-PortListening $backendPort)) {
  $backendCmd = "Set-Location '$backendDir'; `$env:BACKEND_PORT='$backendPort'; & '$pythonExe' -m app.server"
