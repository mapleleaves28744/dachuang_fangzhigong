...
if ($redisReady) {
  $celeryRunning = Get-CimInstance Win32_Process | Where-Object {
    $_.CommandLine -match "celery" -and $_.CommandLine -match "app\.server:celery_client"
  }

  if (-not $celeryRunning) {
    $celeryCmd = "Set-Location '$backendDir'; `$env:CELERY_BROKER_URL='redis://127.0.0.1:6379/0'; `$env:CELERY_RESULT_BACKEND='redis://127.0.0.1:6379/1'; & '$pythonPathEscaped' -m celery -A app.server:celery_client worker -l info -P solo"
...
if (-not (Is-PortListening $backendPort)) {
  $backendCmd = "Set-Location '$backendDir'; `$env:BACKEND_PORT='$backendPort'; & '$pythonPathEscaped' -m app.server"
