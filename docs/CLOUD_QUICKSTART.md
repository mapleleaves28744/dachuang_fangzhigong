...
编辑 `backend/.env`，或新建 `backend/config/.env`，最少确保：
...
2. 问答失败
- 先执行 `./scripts/check-dev-stack.sh`
- 再看 `backend/.env` 或 `backend/config/.env` 的 AI 配置是否完整
- 检查 QWEN_API_KEY 是否有效

3. 启动失败
- 看日志目录 `backend/logs/` 和 `frontend/logs/`
- 重点查看 `backend/logs/backend.log` 和 `backend/logs/celery.log`
